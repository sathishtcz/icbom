...
const Header = () => {
    ...
    //     `${navlocation.pathname === path ? 'text-[#FFDB00]' : 'text-black hanuman-medium'
    //     }`;
        ...
        <div ref={menuRef} className={`w-full fixed top-0 z-20  transition-all duration-300 ${scrolled ? "bg-gradient-to-r from-[#176E68] to-[#26B07C] shadow-md" : "bg-gradient-to-r from-[#176E68] to-[#26B07C]"}`}>
            <div className="mx-auto w-full xl:max-w-[1450px] md:max-w-[920px] flex items-center justify-between px-3">
                ...
                <div className="lg:flex hidden items-center">
                    <ul className={`flex gap-12 h-full items-center justify-center hanuman-medium text-[18px] ${scrolled ? 'text-white ' : 'text-white'}`}>
                        <li >
                            <Link to="/" >Home</Link>
                        </li>
                        <li ref={dropdown1Ref} className="cursor-pointer relative ">
                            ...
                            {dropdown1 && (
                                <div className="absolute bg-white border border-gray-600 shadow-lg mt-2 rounded ">
                                    <ul className="p-2 text-[18px] max-w-none w-full whitespace-nowrap">
                                        {Aboutus.map((link, index) => (
                                            <Link key={index} onClick={() => setDropdown1(false)} to={link.path} className="!w-full">
                                                <li className="px-4 py-2 text-gray-700 hanuman-medium">{link.name}</li>
                                            </Link>
                                        ))}
                                    </ul>
                                </div>
                ...
                >
                    <GiHamburgerMenu className={`ml-12 text-3xl cursor-pointer duration-300 m-1.5 ${scrolled ? 'text-white ' : 'text-white'} `} />
                </div>
            </div>
